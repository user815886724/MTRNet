...
class U_former(nn.Module):
    def __init__(self, image_size=128, in_channels=3, embed_dim=32, win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
        ...
        if csff:
            ...
            self.csff_decoder2 = nn.Conv2d(embed_dim*8, embed_dim*4, kernel_size=1, bias=qkv_bias)
            self.csff_decoder1 = nn.Conv2d(embed_dim*4, embed_dim*2, kernel_size=1, bias=qkv_bias)
            self.csff_decoder0 = nn.Conv2d(embed_dim*2, embed_dim, kernel_size=1, bias=qkv_bias)
    ...
    def forward(self, x,  mask=None, encoder_outs=None, decoder_outs=None):
        ...
        if self.csff and encoder_outs is not None and decoder_outs is not None:
            conv0 = conv0.permute(0,3,1,2) + self.csff_encoder0(encoder_outs[0].permute(0,3,1,2)) + \
                    self.csff_decoder0(decoder_outs[2].permute(0,3,1,2))
            conv0 = conv0.permute(0, 2, 3, 1)
        pool0 = self.dowsample_0(conv0)

        conv1 = self.encoder_layer1(pool0, mask=mask)
        if self.csff and encoder_outs is not None and decoder_outs is not None:
            conv1 = conv1.permute(0,3,1,2) + self.csff_encoder1(encoder_outs[1].permute(0,3,1,2)) + \
                    self.csff_decoder1(decoder_outs[1].permute(0,3,1,2))
            conv1 = conv1.permute(0, 2, 3, 1)
        pool1 = self.dowsample_1(conv1)

        conv2 = self.encoder_layer2(pool1, mask=mask)
        if self.csff and encoder_outs is not None and decoder_outs is not None:
            conv2 = conv2.permute(0,3,1,2) + self.csff_encoder2(encoder_outs[2].permute(0,3,1,2)) + \
                    self.csff_decoder2(decoder_outs[0].permute(0,3,1,2))
...
class TransformerVision(nn.Module):
    def __init__(self,dim, input_resolution, num_heads, win_size=8, shift_size=0, mlp_ratio=4., qkv_bias=True,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, active_layer=active_layer, drop=drop) if token_mlp=='ffn' else LeFF(dim,mlp_hidden_dim,active_layer=active_layer, drop=drop)
